...
spec :: Spec
spec = do
    describe "Simple Markup parsing tests" $ do
        simple
        multiLine

simple :: Spec
simple = do
    it "empty" . shouldBe (parse "") $ []
    it "paragraph" . shouldBe (parse "hello world") $ [Paragraph "hello world"]
    it "heading 1" . shouldBe (parse "* Heading 1") $ [Heading 1 "Heading 1"]
    it "code" . shouldBe (parse "> main = putStrLn \"Hello, World!\"")
        $ [CodeBlock ["main = putStrLn \"Hello, World!\""]]

multiLine :: Spec
multiLine = do
    describe "Multi-line tests" $ do
        it "example3" . shouldBe (parse example3) $ example3Result
        it "example4" . shouldBe (parse example4) $ example4Result
